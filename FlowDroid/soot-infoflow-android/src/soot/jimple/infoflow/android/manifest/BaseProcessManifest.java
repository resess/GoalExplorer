		...
		for (AXmlNode node : list) {
			...
			if (attr != null ) {
				...
				if(indexOfClassName > -1){
					if (((AXmlAttribute<?>) attr).getValue().equals(name.substring(indexOfClassName)))
						return node;
					if (((AXmlAttribute<?>) attr).getValue().equals(name.substring(indexOfClassName+1)))
						return node;
				}
				if(getPackageName() != null && cache_PackageName.length() > 0 && cache_PackageName.length() < name.length()){
