...
JAVA_CLASSPATH="./soot-github/lib/soot-develop.jar:\
./soot-github/libs/AXMLPrinter2.jar:\
./server/ServerNew.jar:"
...
function compile_app {
	...
	# update android project (SDK version 19)
...
function build_fsm {
	...
	if [ "$PROJECT_TYPE" = "ant" ]; then
		DebugApk=`ls ${APP_SOURCE_DIR}/bin/*-instrumented.apk`
	else
		DebugApk=$APK_PATH
	fi
	...
	SOOT_CMD="SocketServer ${APP_SOURCE_DIR} --apk-name ${DebugApk%.apk} -w -keep-line-number -allow-phantom-refs -src-prec apk -android-jars ${ANDROID_JARS_PATH} -f J -d ${SOOT_OUT_DIR} ${PROCESS_THIS}"
...
function build_fsm_apk {
	...
	SOOT_CMD="SocketServer $OutputDir --apk-name ${DebugApk%.apk} -w -keep-line-number -allow-phantom-refs -src-prec apk -android-jars $ANDROID_JARS_PATH -f J -d ${SOOT_OUT_DIR} $PROCESS_THIS"
...
function MCMC_sample {
	...
	MCMC_CMD="MCMCSampler --test ${APP_SOURCE_DIR}"

	java -Xss50m -Xmx1500m -classpath ${JAVA_CLASSPATH} ${MCMC_CMD}
}
...
function MCMC_sample_apk {
	...
	MCMC_CMD="MCMCSampler --test $OutputDir"
...
if [ "${OPTION}" = "compile" ]; then
	...
	# "app-name/", "app-name-instr/", "app-name-apks/"
	echo "MCMC-droid"
	echo "Current Configuration: compile apps as \"android-19\" "
