...
class Agent
    ...
    def initialize(emulator_name, emulator_opt, socket_port, config, closed_source_apk, disable_coverage, manager)
        ...
        # @login_setting = @config.get_login_setting()
        @login_setting = true
...
#        puts "pkg:#{pkg}"
    ...
    def rip_app (act, pkg, test_sequence)
        ...
        # check page, TODO comment this when you are not testing real-world apps that requires login (e.g., k9mail)
        # UTIL.execute_shell_cmd("python ./bin/events/dump.py #{@emulator_serial} /tmp/test.xml")
        # if @login_setting then
        #   puts "Checking login widgets in agent.rb"
        #   UTIL.need_login(@emulator_serial, "/tmp/test.xml", pkg)
        # end
    ...
    def working_loop ()
      ...
      puts "[debug] activity: #{act}; package: #{pkg}"
